...
open class ThingsManager {
    interface API {
        companion object {
            ...
            const val entitiesUnderGroup = "/api/entityGroup/{entityGroupId}/entities?limit=100"
            const val device = "/api/device" // GET for get device, POST for add Device
            const val enityGroup = "/api/entityGroup"
            const val getDeviceFromenityGroup = "/api/entityGroup"// GET for get device, POST for add Device
            const val getDeviceindexval = "/api/plugins/telemetry/DEVICE"// GET for get device, POST for add Device
    ...
    companion object {
        ...
        fun saveEntityGroup(c: Context, groupName: String, description: String = "", displayName: String = "") {
            try {
                // Generating Req
                val client =
                    JsonClient(
                ...
                client.execute(
                    c as ResponseListener, jsonObject = jsonObject, responseType = Device::class.java
                )
            } catch (e: Exception) {
                throw e
            }
        ...
        fun getDevice(c: Context, l: ResponseListener, deviceId: String) {
            try {
                // Generating Req
                val client = RestClient(
                    c, Request.Method.GET, constructUrl("${API.device}/$deviceId"), API.device.hashCode()
                )
                fillCommons(c = c, r = client)
                client.execute(l, responseType = Device::class.java)
            } catch (e: Exception) {
...
//                    constructUrl("${API.telemetryData}/$entityValType/$entityId/$Keys/$fromDate/$toDate/$interval/$limit/$agg"),
                    constructUrl("${API.telemetryData}/$entityId/values/timeseries?limit=100?agg=NONE?keys=" + "$Keys?startTs=$fromDate?endTs=" + "$toDate"),
        ...
        fun getdevicelist(c: Context, l: ResponseListener, entityGroupId: String) {
            try {
                ...
                val client = RestClient(
                    c,
                    Request.Method.GET,
                    constructUrl("${API.getDeviceFromenityGroup}/$entityGroupId" + "/entities?limit=100&ascOrder=false"),
        ...
        fun getDeviceCredentialsByDeviceId(c: Context, deviceId: String) {
            try {
                ...
                val client = RestClient(
                    c,
                    Request.Method.GET,
                    constructUrl(API.getDeviceCredentialsByDeviceId.replace("{deviceId}", deviceId)),
         ...
         */
        fun callRPCTwoWay(c: Context, l: ResponseListener, deviceId: String, jsonObject: JSONObject) {
            try {
                // Generating Req
                val client = JsonClient(
                    c, Request.Method.POST,
                    constructUrl(API.rpcTwoWay.replace("{deviceId}", deviceId)), API.rpcTwoWay.hashCode()
                )
         ...
         * Arm / DisArm device using rpcTwoWay call
         */
        fun callRPCTwoWayGetState(c: Context, l: ResponseListener, deviceId: String, jsonObject: JSONObject) {
            try {
                // Generating Req
                val client = JsonClient(
                    c,
         ...
         */
        fun addAttributedeviceindex(c: Context, l: ResponseListener, deviceId: String, deviceindex: String) {
            try {
                // Generating Req`
                val client = JsonClient(
                    c, Request.Method.POST,
         ...
         */
        fun addAttributedevicename(c: Context, l: ResponseListener, deviceId: String, devicename: String) {
            try {
                // Generating Req`
                val client = JsonClient(
                    c, Request.Method.POST,
